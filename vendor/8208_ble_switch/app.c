...
#if (BLE_REMOTE_PM_SIMPLE_EN && (0 == RC_BTN_ENABLE))
#if(PCBA_8208_SEL == C1T261A30_V1)
#define KEY_PRESS_DEBOUNCE_FILTER_CNT		(4)
#elif(PCBA_8208_SEL == C1T261A5_V1)
#define KEY_PRESS_DEBOUNCE_FILTER_CNT		(12)
#endif
u8 key_pressing_cnt = KEY_PRESS_DEBOUNCE_FILTER_CNT;
#endif

static u32 smart_home_keyScanTick;
/**
 * @brief       This function button pro
 * @return      none
 * @note        
 */
void proc_ui_smart_home()
{
	...
	if(clock_time_exceed(smart_home_keyScanTick, 8000)){
		smart_home_keyScanTick = clock_time();
	}
	else{
		return;
	}
	...
	#if 1
	{
		...
		u8 st_sw1 = kb_event.keycode[0];
		key_pressing_flag = key_not_released;
		if(st_sw1 == VK_1 || (key_not_released&&(st_sw1_step == 1))){
			if(0 == st_sw1_step){
				st_sw1_step = 1;
				...
				if(clock_time_exceed(long_tick1, 3000*1000)){
					...
					if(is_test_key_function_2()){
						test_smart_home_cmd_tx_scene_add();
					}else
				#endif
					{
						smart_home_toggle_switch_mode();
					}
				}
			}else{
				// nothing to do
			...
			#if TEST_SWITCH_KEY_FUNCTION_2_EN
				if(is_test_key_function_2()){
					test_smart_home_cmd_tx_scene_call();
				}else
			#endif
				{
					test_smart_home_cmd_tx_onoff_toggle();
				}
...
#if 1
	{
		...
		u8 st_sw2 = kb_event.keycode[0];
		key_pressing_flag = key_not_released;
		if(st_sw2 == CR_VOL_UP || (key_not_released&&(st_sw2_step == 1))){
			if(0 == st_sw2_step){
				st_sw2_step = 1;
			...
			#endif
				{
					static u16 pair_request_cnt; pair_request_cnt++;
					smart_home_tx_pair_request_switch();
				}
...
#elif (PCBA_8208_SEL == C1T261A5_V1)
	static u8 st_sw4_step_off, st_sw5_step_off;	
	static u32 long_tick4_off;
	static u8	kb_last[2];
	key_pressing_flag = key_not_released;
	if(det_key){
		if(kb_event.keycode[0] == RC_KEY_L){		
			smart_home_cmd_tx_ct(GROUP_ID_SWITCH_G_F000, LIGHTNESS_CT_STEP, LIGHT_SET_MODE_DECREASE, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_R){
			smart_home_cmd_tx_ct(GROUP_ID_SWITCH_G_F000, LIGHTNESS_CT_STEP, LIGHT_SET_MODE_INCREASE, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_1_ON){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 0, L_ON, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_1_OFF){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 0, L_OFF, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_2_ON){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 1, L_ON, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_2_OFF){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 1, L_OFF, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_3_ON){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 2, L_ON, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_3_OFF){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 2, L_OFF, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_4_ON){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 3, L_ON, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_4_OFF){
			smart_home_cmd_tx_onff(GROUP_ID_APP_START + 3, L_OFF, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_UP){
			smart_home_cmd_tx_lightness(GROUP_ID_SWITCH_G_F000, LIGHTNESS_CT_STEP, LIGHT_SET_MODE_INCREASE, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_DN){
			smart_home_cmd_tx_lightness(GROUP_ID_SWITCH_G_F000, LIGHTNESS_CT_STEP, LIGHT_SET_MODE_DECREASE, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_A_ON){
			smart_home_cmd_tx_onff(GROUP_ID_SWITCH_G_F000, L_ON, 0, NO_ACK);
		}else if(kb_event.keycode[0] == RC_KEY_A_OFF){
			smart_home_cmd_tx_onff(GROUP_ID_SWITCH_G_F000, L_OFF, 0, NO_ACK);
		}
		kb_last[0] = kb_event.keycode[0];
		kb_last[1] = kb_event.keycode[1];
		smart_home_led_event_callback(LED_EVENT_KEY_PRESS);
	}else if(((kb_last[0] == RC_KEY_1_OFF) &&(kb_last[1] == RC_KEY_A_ON)) || \
				((kb_last[1] == RC_KEY_1_OFF) &&(kb_last[0] == RC_KEY_A_ON)) || \
					(key_not_released&&(st_sw4_step_off == 1))){
		
		if(0 == st_sw4_step_off){
				st_sw4_step_off = 1;
				long_tick4_off = clock_time();
		}else if(1 == st_sw4_step_off){
			if(clock_time_exceed(long_tick4_off, 3000*1000)){
				// long pressed
				st_sw4_step_off = 0;
				smart_home_tx_pair_request_switch();
			}
		}
	}else if(((kb_last[0] == RC_KEY_4_OFF) &&(kb_last[1] == RC_KEY_A_ON)) || \
				((kb_last[1] == RC_KEY_4_OFF) &&(kb_last[0] == RC_KEY_A_ON)) || \
					(key_not_released&&(st_sw5_step_off == 1))){
		
		if(0 == st_sw5_step_off){
				st_sw5_step_off = 1;
				long_tick4_off = clock_time();
		}else if(1 == st_sw5_step_off){
			if(clock_time_exceed(long_tick4_off, 3000*1000)){
				// long pressed
				st_sw5_step_off = 0;
				smart_home_tx_del_node_switch();
	    ...
	    st_sw4_step_off = st_sw5_step_off = long_tick4_off = 0; // init
    }
#endif
